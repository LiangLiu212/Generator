# INCL26_09a — INCL26_07a with the Nieves (Z-expansion, no RPA) CC QE model

Created 2026-09-11 as a copy of `INCL26_07a`; the only difference is the CC QE
cross-section model in `ModelConfiguration.xml` (`XSecModel@genie::EventGenerator/QEL-CC`
= `genie::NievesQELCCPXSec/ZExpNoRPA`, as in the SBN `AR25_20i` tune, instead of
`genie::HybridXSecAlgorithm/Unified-CC-INCL`). Runnable as tune id `INCL26_09a_00_000`.
It needs its own cross-section splines (the INCL2607a spline set is keyed by the
Unified-CC-INCL algorithm). The rest of this README is inherited from INCL26_07a.

`INCL26_07a` itself is described below (history kept for reference; `AR23_20i` is the
common ancestor).

## Physics content

 - **1p1h QE (CC), INCL26_09a:** `genie::NievesQELCCPXSec/ZExpNoRPA` (Valencia model,
   Z-expansion axial form factor, RPA off). The QEL-CC event-generation thread is unchanged
   (`genie::QELEventGeneratorINCL/Hybrid`, see below).
 - **1p1h QE (CC), INCL26_07a (for reference):** `genie::HybridXSecAlgorithm/Unified-CC`, which delegates
