...
genie::utils::gsl::INCLFullQELdXSec::INCLFullQELdXSec(const XSecAlgorithmI* xsec_model,
  ...
  fHitNucleonBindingMode( binding_mode ), fMinAngleEM( min_angle_EM )
{
  ...
  //assert( fNuclModel );
  //TODO:  fix this
  fNucleusGen = dynamic_cast<const NucleusGenI*>( fXSecModel->SubAlg( "IntegralNucleusGen" ) );
  assert( fNucleusGen );
}

genie::utils::gsl::INCLFullQELdXSec::~INCLFullQELdXSec()
{
  delete fInteraction;
}

Interaction* genie::utils::gsl::INCLFullQELdXSec::GetInteractionPtr()
{
  return fInteraction;
}

const Interaction& genie::utils::gsl::INCLFullQELdXSec::GetInteraction() const
{
  return *fInteraction;
}

ROOT::Math::IBaseFunctionMultiDim* genie::utils::gsl::INCLFullQELdXSec::Clone(void) const
{
  return new INCLFullQELdXSec(fXSecModel, fInteraction, fHitNucleonBindingMode, fMinAngleEM);
}

unsigned int genie::utils::gsl::INCLFullQELdXSec::NDim(void) const
{
  return 2;
}

double genie::utils::gsl::INCLFullQELdXSec::DoEval(const double* xin) const
{
  // Elements of "xin"
  //
